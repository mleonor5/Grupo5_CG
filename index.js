...
class Ball {
    ...
    leftCanvas() {
        if (this.y >= H + this.R + 5) {    //Se o circulo ultrapassou a borda inferior
            this.y = -this.R
...
function drawTriangle() {
    ...
    ctx.save()
    ctx.setTransform(sW, 0, 0, sH, deltaX, deltaY)
    ctx.fillStyle = "white"
    ctx.beginPath();
...
function init() {
    ...
    for (let i = 0; i < 10; i++) {
        ...
        b.push(new Ball(xInit, yInit, radius, direction, velocity, color,sW,sH))
    }
}
...
function render() {
    if (resize) {
        ...
        ctx.beginPath()
        ctx.arc(W/2, H/2, 31, 0, 2 * Math.PI)
        ctx.strokeStyle = 'red'
        ctx.stroke()
